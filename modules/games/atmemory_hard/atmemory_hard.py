...
class memory_GUI( wx.Frame ):
	...
	def initializeParameters( self ):
		...
		self.gamerand = np.random.randint(14)
		print self.gamerand
		self.indexes = np.arange( 1 + self.gamerand, self.gamesize[ 0 ] * self.gamesize[ 1 ] / 2 + 1 +self.gamerand)
	...
	#-------------------------------------------------------------------------
		...
		for i in self.indexes:
			i = str( i )
			file = self.path + 'multimedia/games/atmemory_hard/' + i + '.png'
		...
		if self.control == 'tracker':
			if self.pressFlag == False:
				...
				if label == -1.0 and not self.winstate: # checking
					...
					self.updateGui(self.rowIteration, self.columnIteration,str(int(self.game.displayfield[self.rowIteration, self.columnIteration-1])+self.gamerand))
