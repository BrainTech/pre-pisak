...
class speller( wx.Frame ):
	...
	def initializeParameters(self):
	    ...
	    self.labels = [ 'A O U W K J H Ą I Y R C M G Ę Ś E N T P B Ł Ż Ń Z B D L F Ó Ć Ź UNDO SPEAK SAVE SPACJA SPECIAL_CHARACTERS OPEN EXIT'.split( ), '1 2 3 4 5 6 7 8 9 0 + - * / = % $ & . , ; : " ? ! @ # ( ) [ ] { } < > ~ UNDO SPEAK SAVE SPACJA OPEN EXIT'.split( )]
	...
	def createGui(self):
		...
		for index_1, item in enumerate( self.labels[ 0 ][ :-7 ] ):
			b = bt.GenButton( self, -1, item, name = item, size = ( 0.985*self.winWidth / self.numberOfColumns[ 0 ], 0.71 * self.winHeight / self.numberOfRows[ 0 ] ) )
		...
		for index_2, item in enumerate( self.labels[ 0 ][ -7 : -4 ], start = 1 ):
		...
		for item in ( self.labels[ 0 ][ -4 ], ):
			b = bt.GenButton( self, -1, item, name = item, size = ( 2 * 0.985*self.winWidth / self.numberOfColumns[ 0 ], 0.71 * self.winHeight / self.numberOfRows[ 0 ] ) )
			b.SetFont( wx.Font( 35, wx.FONTFAMILY_ROMAN, wx.FONTWEIGHT_LIGHT,  False ) )
			b.SetBezelWidth( 3 )
			b.SetBackgroundColour( self.backgroundColour )
			b.SetForegroundColour( self.textColour )
		...
		for index_3, item in enumerate( self.labels[ 0 ][ -3: ], start = 3 ):
			b = bt.GenBitmapButton( self, -1, name = item, bitmap = self.labelBitmaps[ item ] )
			b.SetBackgroundColour( self.backgroundColour )
			b.SetBezelWidth( 3 )
                        b.Bind( event, self.onPress )
		...
		if self.control == 'tracker':
			...
			if self.numberOfPresses == 1:
				...
				if self.flag == 'rest':
					...
					elif self.rowIteration == self.numberOfRows[ self.subSizerNumber ] and self.subSizerNumber == 0:
					...
					elif self.rowIteration == self.numberOfRows[ self.subSizerNumber ] and self.subSizerNumber == 1:
					...
					elif self.rowIteration == self.numberOfRows[ self.subSizerNumber ] - 1:
		...
		if self.control == 'tracker':
			...
			if self.flag == 'row':
				...
				if self.countRows == self.maxNumberOfRows:
					...
					elif self.rowIteration == self.numberOfRows[ self.subSizerNumber ]-1 and self.subSizerNumber == 0:
						self.countRows += 1
					...
					elif self.rowIteration == self.numberOfRows[ self.subSizerNumber ]-1 and self.subSizerNumber == 1:
						self.countRows += 1
				...
				if self.countColumns == self.maxNumberOfColumns:
					...
					if self.subSizerNumber == 0:
						if self.columnIteration == self.numberOfColumns[ self.subSizerNumber ] - 1 or ( self.columnIteration == self.numberOfColumns[ self.subSizerNumber ]-1 and self.rowIteration == self.numberOfRows[ self.subSizerNumber ]-1 ):
							self.countColumns += 1

						if self.columnIteration == self.numberOfColumns[ self.subSizerNumber ] or ( self.columnIteration == self.numberOfColumns[ self.subSizerNumber ]-1 and self.rowIteration == self.numberOfRows[ self.subSizerNumber ]-1 ):
							self.columnIteration = 0

					elif self.subSizerNumber == 1:
						if self.columnIteration == self.numberOfColumns[ self.subSizerNumber ] - 1 or ( self.columnIteration == self.numberOfColumns[ self.subSizerNumber ]-3 and self.rowIteration == self.numberOfRows[ self.subSizerNumber ]-1 ):
							self.countColumns += 1

						if self.columnIteration == self.numberOfColumns[ self.subSizerNumber ] or ( self.columnIteration == self.numberOfColumns[ self.subSizerNumber ]-2 and self.rowIteration == self.numberOfRows[ self.subSizerNumber ]-1 ):
							self.columnIteration = 0
